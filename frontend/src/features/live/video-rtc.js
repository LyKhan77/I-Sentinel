...
export class VideoRTC extends HTMLElement {
    ...
    onconnect() {
        ...
        const socket = this.ws = new WebSocket(this.wsURL);
     ...
     */
    onopen(socket = this.ws) {
        // Events from an obsolete socket must not touch the replacement connection.
     ...
     */
    onclose(socket = this.ws) {
        // A close from an obsolete socket must not restart the replacement one.
        if (socket !== this.ws) return false;
